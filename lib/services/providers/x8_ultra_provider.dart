...
/// X8 Ultra Watch Provider - BLE-based smartwatch with comprehensive sensor support
/// Similar to X10 but optimized for X8 Ultra specifications
class X8UltraProvider extends SensorProvider {
  // X8 Ultra proprietary BLE UUIDs
  ...
  @override
  SensorSource get sourceType => SensorSource.x8Ultra;

  @override
  String get displayName => 'X8 Ultra Watch';
  ...
  Future<bool> requestPermissions() async {
    try {
      debugPrint('X8 Ultra: Requesting BLE permissions');
      return true;
    } catch (e) {
      debugPrint('X8 Ultra: Permission error: $e');
      return false;
    }
  }
  ...
  Future<void> stopListening() async {
    ...
    debugPrint('X8 Ultra: Sensor updates stopped');
  }
  ...
  Future<int?> getBatteryLevel() async {
    ...
    try {
      ...
      debugPrint('X8 Ultra: Error getting battery: $e');
      return null;
    }
  }

  /// Connect to X8 Ultra Watch device
  Future<bool> connectToDevice(BluetoothDevice device) async {
    try {
      _device = device;
      await device.connect(timeout: const Duration(seconds: 10));
      debugPrint('X8 Ultra: Connected to ${device.platformName}');
      ...
      debugPrint('X8 Ultra: Connection error: $e');
      return false;
    }
  }
  ...
  void _subscribeToBleNotifications() {
    ...
    _characteristicSubscription = _notifyChar!.onValueReceived.listen(
      (data) {
        _processX8UltraData(data);
      },
      onError: (error) {
        debugPrint('X8 Ultra: Notification error: $error');
      },
    );
  }

  void _processX8UltraData(List<int> data) {
    try {
      // X8 Ultra proprietary data format
      ...
      // Bytes 13-14: Step count (uint16)
      // Byte 15: Battery (%)

      if (data.length < 16) {
        debugPrint('X8 Ultra: Invalid packet size ${data.length}');
        return;
      }
      ...
      if (dataType == 0x01) {
        ...
        final stepCount = ((data[13] << 8) | data[14]);
        final battery = data[15];
        ...
        debugPrint(
          'X8 Ultra: HR=$heartRate SpO2=$spo2 Temp=$temperature Steps=$stepCount Battery=$battery%',
        );
      }
    } catch (e) {
      debugPrint('X8 Ultra: Data processing error: $e');
    }
  }

  /// Get list of available X8 Ultra devices via BLE scan
    ...
    try {
      ...
      final subscription = FlutterBluePlus.scanResults.listen((results) {
        for (final result in results) {
          if (result.device.platformName.contains('X8') ||
              result.advertisementData.advName.contains('X8')) {
      ...
      debugPrint('X8 Ultra: Found ${devices.length} devices');
      return devices;
    } catch (e) {
      debugPrint('X8 Ultra: Scan error: $e');
      return [];
    }
  }
}
